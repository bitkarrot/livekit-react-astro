...
const useWarnAboutMissingStyles = () => {
  React.useEffect(() => {
    ...
    const hasStyles =
      document.querySelector(".lk-video-conference") !== null ||
      document.styleSheets.length > 0;
    if (!hasStyles) {
      console.warn(
        "LiveKit component styles might be missing. Make sure to import the CSS.",
      );
    }
  }, []);
};
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      const trackReference =
        screenShareTracks[0] as unknown as TrackReferenceOrPlaceholder;
    ...
    if (focusTrack && !isTrackReference(focusTrack)) {
      ...
      const updatedFocusTrack = tracks.find(
        (tr) =>
          tr.participant?.identity ===
            (focusTrack.participant as any)?.identity &&
      ...
      if (
        ...
        const trackReference =
          updatedFocusTrack as unknown as TrackReferenceOrPlaceholder;
        layoutContext.pin.dispatch?.({ msg: "set_pin", trackReference });
      }
    }
  }, [
