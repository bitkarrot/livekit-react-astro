...
export const ZapPaymentModal = ({ lightningAddress, onClose }: { lightningAddress: string; onClose: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        "Failed to process donation. Please check the Lightning Address.",
      );
      console.error(err);
    }
  };
    ...
    React.useEffect(() => {
      return () => {
        // No-op cleanup to ensure intervals are cleared if component unmounts
      };
    }, []);

    // Removed 'if (!isOpen) return null;' as it may be causing a lint error or is redundant
